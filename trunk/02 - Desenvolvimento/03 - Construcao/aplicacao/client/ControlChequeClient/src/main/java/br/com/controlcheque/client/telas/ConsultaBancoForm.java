...
public class ConsultaBancoForm extends ConsultaBaseForm implements InterfaceManutencaoSimples {
    ...
    private void preencherTabela(int p_ordenacao) throws Exception {
        try {
            ...
            g_listaBancos = bancoServerImpl.getTodosBancos();
    ...
    /**
     * Metodo que atualiza o Banco passado como parametro na lista de Bancos da Grid
     * @param auxCliente
     */
    ...
    public void deletar() {
        try {
            ...
            bancoServerImpl.deletarBanco(banco);
        } catch (Exception ex) {
            Logger.getLogger(ConsultaBancoForm.class.getName()).log(Level.SEVERE, null, ex);
            //JOptionPane.showMessageDialog(rootPane, MensagemClientBanco.getMensagemErro(ex.getMessage()), "Erro", 2);
        }
    }

    public Object deletarBanco() throws Exception {
        try {
            Banco banco = new Banco();
            banco.setId(tb_bancos.getValueAt(tb_bancos.getSelectedRow(), 0).toString());
            //chamar metodo do server para remover banco
            return bancoServerImpl.deletarBanco(banco);
        } catch (Exception ex) {
            Logger.getLogger(ConsultaBancoForm.class.getName()).log(Level.SEVERE, null, ex);
            throw ex;
        }
    ...
    public void salvar() {
        try {
            ...
            //salvar no server
            this.bancoServerImpl.salvarListaBancos(this.g_listaBancos);
    ...
    private void actionGravarBanco() {
    ...
    private void actionDeletarBanco() {
        final JDialogProcessando aguarde = new JDialogProcessando(this, "Aguarde...", "Processando...");
        aguarde.setVisible(true);
    ...
    private void eventDeletarBanco() {
        final JDialogProcessando aguarde = new JDialogProcessando(this, "Aguarde...", "Processando...");
        aguarde.setVisible(true);
