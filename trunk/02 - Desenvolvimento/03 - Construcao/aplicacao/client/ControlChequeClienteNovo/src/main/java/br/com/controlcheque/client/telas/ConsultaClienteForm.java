 ...
 * @author Jose Robson
 */
public class ConsultaClienteForm extends ConsultaBaseForm {
            ...
            if (lstClientes != null
                ...
                for (int i = 0; i < lstClientes.size(); i++) {
                    dtm.addRow(new Object[]{
                                ...
                                lstClientes.get(i).getCidadeEndereco(),
                                lstClientes.get(i).getRuaEndereco(),
                                lstClientes.get(i).getBairroEndereco(),
                                lstClientes.get(i).getNumeroEndereco()
                            });
                }
    ...
    private void atualizarClienteNaLista(Cliente auxCliente) {
        ...
        for (Cliente cliente : lstClientes) {
            if (auxCliente.getId().equals(cliente.getId())) {
                ...
                cliente.setCidadeEndereco(auxCliente.getCidadeEndereco());
                cliente.setTelefone(auxCliente.getTelefone());
                cliente.setBairroEndereco(auxCliente.getBairroEndereco());
                cliente.setRuaEndereco(auxCliente.getRuaEndereco());
                cliente.setNumeroEndereco(auxCliente.getNumeroEndereco());
            }
        }
    }
    ...
    public void preDeletar() {
        if (tb_clientes.getSelectedRow() != -1) {
            ...
            if (JOptionPane.showConfirmDialog(rootPane, "Remover cliente?", "Remover", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE)
                    == JOptionPane.YES_OPTION) {
                final JDialogProcessando aguarde = new JDialogProcessando(this, "Aguarde...", "Processando...");
                aguarde.setVisible(true);

                SwingWorker<Cliente, Void> worker = new SwingWorker<Cliente, Void>() {
                    ...
                    protected void done() {
                        try {
                            ...
                            tb_clientes.getDefaultEditor(String.class).cancelCellEditing();
                            initialize();
    ...
    public Object deletar() throws Exception {
        try {
            ...
            cliente = (Cliente) clienteServer.deletar(cliente);
            removerClienteListaPorId(cliente.getId());
            return cliente;
        } catch (Exception ex) {
            LogUtil.logDescricaoErro(this.getClass(), ex);
            throw ex;
    ...
    public Object salvar() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < tb_clientes.getRowCount(); i++) {
                ...
                auxCliente.setCidadeEndereco(tb_clientes.getValueAt(i, 4).toString());
                auxCliente.setRuaEndereco(tb_clientes.getValueAt(i, 5).toString());
                auxCliente.setBairroEndereco(tb_clientes.getValueAt(i, 6).toString());
                auxCliente.setNumeroEndereco(tb_clientes.getValueAt(i, 7).toString());
                atualizarClienteNaLista(auxCliente);
            }

            //salvar no server
            return this.clienteServer.salvarListObject(this.lstClientes);
        } catch (Exception ex) {
            LogUtil.logDescricaoErro(this.getClass(), ex);
            throw ex;
        }
    ...
    private void removerClienteListaPorId(String id) {
        ...
        this.lstClientes.remove(remover);
    }
    /**
     * ***********************************************************************
     * FIM EVENTOS
