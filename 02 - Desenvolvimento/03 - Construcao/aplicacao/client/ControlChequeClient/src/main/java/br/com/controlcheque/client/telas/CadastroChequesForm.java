...
public class CadastroChequesForm extends CadastroBaseForm implements InterfaceCadastroCompleto {
    ...
    private List<Banco> gListBancos;
    private List<Cliente> gListaClientes;
     ...
     * INICIO CONSTRUTORES
     *************************************************************************/
    /** Creates new form CadastroChequesForm */
    ...
    @Override
    public void close() {
        App.telaPrincipal.setFormCadastroCheque(null);
        this.dispose();
    }
    ...
    public void pushToModel() throws ClientChequeException {
        ...
        try {
            cheque.setDataBomPara(df.parse(txt_bomPara.getText()));
        } catch (ParseException ex) {
            txt_bomPara.requestFocus();
            throw new ClientChequeException(ClientConstantes.CODIGO_ERRO_DATA_BOMPARA_INVALIDA, ex);
        }
        try {
            if (!txt_dataEmissao.getText().isEmpty()) {
                cheque.setDataEmissao(df.parse(txt_dataEmissao.getText()));
            }
        } catch (ParseException ex) {
            txt_dataEmissao.requestFocus();
            throw new ClientChequeException(ClientConstantes.CODIGO_ERRO_DATA_EMISSAO_INVALIDA, ex);
        }
    ...
    public void salvar() {
        try {
            if (validarTela()) {
                ...
                String id = cheque.getId();
                if (chequeServerImpl.mantemCheque(cheque)) {
                    habilitaBotaoNovo(true);
                    this.txt_banco.requestFocus();
                    if (id == null) {
                        JOptionPane.showMessageDialog(rootPane, MensagemClientCheque.MENSAGEM_CADASTRO_SUCESSO_CHEQUE, "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(rootPane, MensagemClientCheque.MENSAGEM_ALTERAR_SUCESSO_CHEQUE, "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                    }
                }
            }
        } catch (Exception ex) {
            //exibir Mensagem
            LogUtil.logDescricaoErro(this.getClass(), ex);
            JOptionPane.showMessageDialog(rootPane, MensagemClientCheque.getMensagemErro(ex.getMessage()), "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public boolean validarTela() {
        ...
        if (lblNomeBanco.getText().trim().isEmpty()) {
    ...
    public void initialize() {
        try {
            ...
            LogUtil.logDescricaoErro(this.getClass(), ex);
            JOptionPane.showMessageDialog(rootPane, MensagemClientGenerica.MENSAGE_ERRO_INICIALIZAR_TELA, "Atenção", 2);
            this.dispose();
        }
     ...
     * @throws Exception
     */
    private void setListaBanco() throws Exception {
        try {
            BancoServerImpl bancoServerImpl = new BancoServerImpl();
            gListBancos = bancoServerImpl.getTodosBancos();
        } catch (Exception ex) {
            throw ex;
        }
    }
     ...
     * @throws Exception
     */
    private void setListaCliente() throws Exception {
        ClienteServerImpl clienteServerImpl = new ClienteServerImpl();
        gListaClientes = clienteServerImpl.getTodosClientes();
    }
    ...
    /**
     * metodo para preencher o campo codigo
     * @param codigo
     */
    ...
    private void buscarCliente(String pNome, JLabel pLabel) {
        ...
        pLabel.setText(null);
        if (!lnome.isEmpty()) {
            if (gListaClientes != null) {
                for (Cliente cliente : gListaClientes) {
    ...
    private void initComponents() {
        ...
        lblRecebiDe.setFont(new java.awt.Font("Arial", 1, 11)); // NOI18N
        lblRecebiDe.setText("Recebi De");

        lblPasseiPara.setFont(new java.awt.Font("Arial", 1, 11)); // NOI18N
                        ...
                        .addGroup(jPanelDadosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanelDadosLayout.createSequentialGroup()
                                .addGroup(jPanelDadosLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    ...
                                    .addComponent(lblRecebiDe, javax.swing.GroupLayout.PREFERRED_SIZE, 225, javax.swing.GroupLayout.PREFERRED_SIZE)
